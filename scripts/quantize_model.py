...
def main():
    ...
    comparison = compare_quantized_model(
        student_fp32, student_int8, holdout_loader, device="cpu"
    )
    ...
    logger.info("-" * 60)
    logger.info(f"{'Size (MB)':<25} {comparison['fp32']['size_mb']:>12.2f} {comparison['int8']['size_mb']:>12.2f} {-comparison['delta']['size_reduction']*100:>+9.1f}%")
    logger.info(f"{'Latency (ms)':<25} {comparison['fp32']['latency_ms']:>12.2f} {comparison['int8']['latency_ms']:>12.2f} {comparison['delta']['latency_speedup']:>9.2f}x")
    logger.info(f"{'ROC-AUC':<25} {comparison['fp32']['roc_auc']:>12.4f} {comparison['int8']['roc_auc']:>12.4f} {comparison['delta']['delta_roc_auc']:>+10.4f}")
    logger.info(f"{'ECE':<25} {comparison['fp32']['ece']:>12.4f} {comparison['int8']['ece']:>12.4f} {comparison['delta']['delta_ece']:>+10.4f}")
    ...
    logger.info(f"Mobile target (<{mobile_target} MB): {'✓' if meets_mobile else '✗'} ({int8_size:.2f} MB)")
    logger.info(f"Edge target (<{edge_target} MB): {'✓' if meets_edge else '✗'} ({int8_size:.2f} MB)")
