...
from src.models.architectures import StudentModel, load_teacher_checkpoint
...
def main():
    ...
    train_loader, val_loader = create_dataloaders(train_path, val_path, config=data_config)
    ...
    teacher, _ = load_teacher_checkpoint(args.teacher_ckpt, config=teacher_config, device=device)
    teacher.eval()
    ...
    logger.info(
        f"Meets mobile constraint (<{constraints['mobile_target_mb']} MB): {constraints['meets_mobile_constraint']}"
    )
    ...
    logger.info("-" * 60)
    logger.info(
        f"{'ROC-AUC':<25} {teacher_metrics.roc_auc:>12.4f} {student_metrics.roc_auc:>12.4f} {student_metrics.roc_auc - teacher_metrics.roc_auc:>+10.4f}"
    )
    logger.info(
        f"{'PR-AUC':<25} {teacher_metrics.pr_auc:>12.4f} {student_metrics.pr_auc:>12.4f} {student_metrics.pr_auc - teacher_metrics.pr_auc:>+10.4f}"
    )
    logger.info(
        f"{'F1':<25} {teacher_metrics.f1:>12.4f} {student_metrics.f1:>12.4f} {student_metrics.f1 - teacher_metrics.f1:>+10.4f}"
    )
    logger.info(
        f"{'ECE':<25} {teacher_metrics.ece:>12.4f} {student_metrics.ece:>12.4f} {student_metrics.ece - teacher_metrics.ece:>+10.4f}"
    )
    logger.info(
        f"{'Spec @95% sens':<25} {teacher_metrics.specificity_at_target_sens:>12.4f} {student_metrics.specificity_at_target_sens:>12.4f} {student_metrics.specificity_at_target_sens - teacher_metrics.specificity_at_target_sens:>+10.4f}"
    )
    ...
    logger.info("-" * 60)
    logger.info(
        f"{'Size (MB)':<25} {teacher_deployment.model_size_mb:>12.2f} {student_deployment.model_size_mb:>12.2f}"
    )
    logger.info(
        f"{'Latency (ms)':<25} {teacher_deployment.avg_latency_ms:>12.2f} {student_deployment.avg_latency_ms:>12.2f}"
    )
    logger.info(
        f"{'Throughput (img/s)':<25} {teacher_deployment.throughput_images_per_sec:>12.1f} {student_deployment.throughput_images_per_sec:>12.1f}"
    )
