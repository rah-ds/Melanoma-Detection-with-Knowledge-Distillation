...
from src.models.kd_loss import create_kd_loss, create_teacher_loss
...
class BaseTrainer:
    ...
    def save_checkpoint(
        ...
        if is_best:
            ...
            logger.info(
                f"Saved best checkpoint: epoch {epoch}, ROC-AUC={metrics.get('roc_auc', 0):.4f}"
            )
            return best_path
...
class TeacherTrainer(BaseTrainer):
    ...
    def train(self, num_epochs: int | None = None) -> TrainingHistory:
...
class StudentTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        logger.info(f"KD Config: T={kd_config.temperature}, α={kd_config.alpha}")
